...
export default function Header() {
  ...
  return (
    <header className="absolute top-0 left-0 right-0 z-50 bg-blue-950 pt-4">
          ...
          <nav className="hidden lg:flex items-center space-x-8">
            {desktopNavigation.map((item) => (
              <div key={item.name} className="relative group">
                ...
                {item.dropdown && (
                  <div className="absolute top-full left-0 mt-2 w-52 bg-blue-950/95 border border-white/30 shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                    {item.dropdown.map((subItem) => (
                      <Link
                        key={subItem.name}
                        href={subItem.href}
                        className="block px-4 py-2 text-sm text-white/90 hover:bg-white/10 hover:text-white transition-colors duration-200"
                      >
                        {subItem.name}
                      </Link>
                    ))}
